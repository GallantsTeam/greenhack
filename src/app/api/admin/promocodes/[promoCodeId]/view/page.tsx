...
export default function ViewPromoCodePage() {
  ...
  const formatPromoType = (type?: PromoCode['type'], value_gh?: number | null, product_name?: string, pricing_option_desc?: string, mode_label?: string | null) => {
    if (!type) return 'N/A';
    if (type === 'balance_gh') return `Пополнение баланса: ${value_gh || 0} GH`;
    if (type === 'product') return `Товар: ${product_name || 'Не указан'} ${pricing_option_desc ? `(${pricing_option_desc}${mode_label ? `, ${mode_label}` : ''})` : (mode_label ? `(${mode_label})` : '')}`;
      ...
      <Card className="shadow-lg">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-sm">
          ...
          <div>
            ...
            <p className="text-foreground">{formatPromoType(promoCodeData.type, promoCodeData.value_gh, promoCodeData.product_name, promoCodeData.pricing_option_description, promoCodeData.pricing_option_mode_label)}</p>
          </div>
          <div>
